...
export const FeaturedCard = ({ item: { image, rating, name, painter, price }, onPress }: Props) => {
    ...
    useEffect(() => {
        const fetchPainterName = async () => {
          try {
            
            if (!painter || typeof painter !== 'string' || painter.length > 36) {
              throw new Error('Invalid painter ID');
            }
      
           
            const painterDoc = await databases.getDocument(
              config.databaseId!,
              config.paintersCollectionId!,
              painter
            );
            setPainterName(painterDoc.name);
          } catch (error) {
            console.error('Error fetching painter name:', error);
            setPainterName('Unknown Painter'); 
          }
        };
    ...
    return (
        <TouchableOpacity onPress={onPress} className="flex flex-col items-start w-60 h-80 relative">
            ...
            <View className="flex flex-col items-start absolute bottom-11 mr-1 inset-x-5">
                ...
                <Text className="text-base font-rubik text-white">{painterName}</Text>
    ...
    return (
        <TouchableOpacity onPress={onPress} className="flex-1 w-full px-3 py-4 rounded-lg bg-white shadow-lg shadow-neutral-950 relative mb-4">
            ...
            <View className="flex flex-col mt-2">
                ...
                <Text className="text-xs font-rubik text-black">{painterName}</Text>
